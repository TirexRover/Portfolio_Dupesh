...
function ActionLink({ href, label, icon }: ActionLinkProps) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="no-select inline-flex items-center gap-1.5 rounded-lg border border-white/60 bg-white/70 px-2.5 py-2 text-[var(--text-primary)] shadow-sm transition active:scale-95 sm:gap-2 sm:rounded-xl sm:px-3 sm:hover:-translate-y-0.5 sm:hover:border-primary-300 sm:hover:text-primary-600 dark:hover:text-white dark:hover:border-white/40 dark:border-white/10 dark:bg-white/5 dark:text-white backdrop-blur-sm"
      aria-label={label}
    >
      {icon}
